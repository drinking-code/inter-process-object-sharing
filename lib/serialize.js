export function serialize(value) {
    ...
    if (['string', 'number', 'boolean'].includes(typeof value) || !value) {
        ...
        return Object.fromEntries(Array.from(Object.entries(value)
            .map(([key, value]) => [key, serialize(value)])));
    }
    else if (value.constructor === Map) {
        return {
            $$iposType: 'Map',
            data: Array.from(value.entries()).map(([key, value]) => [key, serialize(value)])
        };
    }
    else {
        if (!value.stringify && !value.serialize)
            throw new Error(`Class: \`${value.constructor.name}\` must have methods to serialize and deserialize objects. (\`.stringify()\`, \`.serialize()\`)`);
